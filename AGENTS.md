...
- **OTel logs**: as of SDK 1.44, `LoggingHandler` lives at `opentelemetry.sdk._logs`. `ai-service/tracing.py` no longer builds SDK providers (the CLI does); it only attaches the handler to the root logger and to `uvicorn.access` (which is `propagate=False` and needs a direct handler for Loki).
