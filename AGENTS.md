...
- **ai-service env knobs**: `OTEL_EXPORTER_OTLP_ENDPOINT` (Alloy `http://alloy:4317`), `OTEL_SERVICE_NAME`, `OTEL_RESOURCE_ATTRIBUTES` (merged into Resource in `tracing.py`), `TRACELOOP_TRACE_CONTENT` (read directly by OpenLLMetry — content capture, default true). Adding env vars that code doesn't consume would be misleading; `OTEL_TRACES/METRICS/LOGS_EXPORTER` etc. do nothing here because providers are built manually in `tracing.py`.
...
- **FastAPI layer is instrumented too**: `FastAPIInstrumentor().instrument_app(app)` (after `setup_otel()` and `app = FastAPI(...)`) makes each `POST /chat/stream` request the root span of the trace, with the LangChain spans nested inside. The only env the LangChain instrumentor reads is `TRACELOOP_TRACE_CONTENT` (verified by grepping 0.62.3 source — there is no `OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT` in this package).
